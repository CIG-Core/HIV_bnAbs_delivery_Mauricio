---
title: "Differential Expression Analysis - AAV young samples"
date: "`r Sys.Date()`"
...
experiment <- "12-21-2023-DE_Analysis_AAV_young" # results with pvalue of 0.01
## experiment <- "06-08-2023-DE_Analysis_AAV_young" # results with pvalue of 0.01
...
metadataPath <- file.path(data_path, "aav_young_samples.csv")
skipLines <- 0
sampleCol <- "Sample"
...
dataDirs <- file.path(data_path, "2022_Kiran", "Mauricio Data")
...
sleuth_results <- sleuth.func(metaData = metadataPath, skipLines=skipLines, sampleCol=sampleCol,
                                     ...
                                     drop_dupGenes = FALSE)
save(sleuth_results, file=file.path(outPath, "sleuth_results.RData"))
...
sleuth_results[["Zero_vs_Late_pca"]]
```

#### Volcano Plot

```{r volcano_2-1, echo=FALSE, warning=FALSE, message=FALSE}
sleuth_results[["Zero_vs_Late_vPlot"]]
```
...
sleuth_results[["Zero_vs_Early_pca"]]
```

#### Volcano Plot

```{r volcano_2-3, echo=FALSE, warning=FALSE, message=FALSE}
sleuth_results[["Zero_vs_Early_vPlot"]]
```
...
sigDET_UpOrDn <- grep("\\w*(Up|Dn)\\b", names(sleuth_results), value=TRUE)
sigDET_UpOrDn <- grep("sigDET", sigDET_UpOrDn, value=TRUE)
for(g1 in sigDET_UpOrDn){
  ## genes <- r1[[g1]]$gene_name
  if(nrow(sleuth_results[[g1]]) > 0){
    ortho_df <- orthogene::convert_orthologs(sleuth_results[[g1]], gene_input="ensembl_gene", 
...
for(a1 in names(ORA)){
  ...
  if(nrow(ORA[[a1]]) !=0){
    ...
    plots_list[[a1]] <- dotplot(reacORkegg, showCategory = 20)
    ggsave(plots_list[[a1]], file.path(cp_path, paste0(a1, "_dotplot.pdf")), device = "pdf",
          units = "cm")
    ## plots_list[[a1]] <- fit
  }
}
...
### Reference Level: Group Zero

#### Zero vs. Late- Up-regulated (KEGG)
```{r enrichment_plot_2_vs_1-KEGG-Up, echo=FALSE, warning=FALSE, message=FALSE}
plots_list$sigDET_Zero_vs_Late_Up_KEGG
```

#### Zero vs. Late- Up-regulated (Reactome)
```{r enrichment_plot_2_vs_1-Reactome-Up, echo=FALSE, warning=FALSE, message=FALSE}
plots_list$sigDET_Zero_vs_Late_Up_Reactome
```

#### Zero vs. Late- Down-regulated (KEGG)
```{r enrichment_plot_2_vs_1-KEGG-Down, echo=FALSE, warning=FALSE, message=FALSE}
plots_list$sigDET_Zero_vs_Late_Dn_KEGG
```

#### Zero vs. Late- Down-regulated (Reactome)
```{r enrichment_plot_2_vs_1-Reactome-Down, echo=FALSE, warning=FALSE, message=FALSE}
plots_list$sigDET_Zero_vs_Late_Dn_Reactome
```

#### Zero vs. Early- Up-regulated (KEGG)
```{r enrichment_plot_2_vs_3-KEGG-Up, echo=FALSE, warning=FALSE, message=FALSE}
plots_list$sigDET_Zero_vs_Early_Up_KEGG
```

#### Zero vs. Early- Up-regulated (Reactome)
```{r enrichment_plot_2_vs_3-Reactome-Up, echo=FALSE, warning=FALSE, message=FALSE}
plots_list$sigDET_Zero_vs_Early_Up_Reactome
```

#### Zero vs. Early- Down-regulated (KEGG)
```{r enrichment_plot_2_vs_3-KEGG-Down, echo=FALSE, warning=FALSE, message=FALSE}
plots_list$sigDET_Zero_vs_Early_Dn_KEGG
```

#### Zero vs. Early- Down-regulated (Reactome)
```{r enrichment_plot_2_vs_3-Reactome-Down, echo=FALSE, warning=FALSE, message=FALSE}
plots_list$sigDET_Zero_vs_Early_Dn_Reactome
```


# Heatmaps

```{r heatmap-cellcycle, include=FALSE, echo=FALSE, warning=FALSE, message=FALSE}
...
## colAnnot <- data.frame(row.names=animalInfo$sample_id, Group=factor(animalInfo$Group, levels = c("Zero", "Early", "Late")))
colAnnot_late <- data.frame(row.names=animalInfo_late$sample_id, Group=factor(animalInfo_late$Group, 
                                                                         levels = c("Zero", "Late")))
colAnnot_early <- data.frame(row.names=animalInfo_early$sample_id, Group=factor(animalInfo_early$Group, 
                                                                         levels = c("Zero", "Early")))
## tpmMat <- sleuth_results$TPM
tpm_late <- tpmMat[, animalInfo_late$Sample]
colnames(tpm_late) <- animalInfo_late$sample_id

tpm_early <- tpmMat[, animalInfo_early$Sample]
colnames(tpm_early) <- animalInfo_early$sample_id

selectGenes <- list()

## read sigDET 
sigDET_late <- sleuth_results$sigDET_Zero_vs_Late
sigDET_late <- sigDET_late[sigDET_late$gene_name != "" ,]
sigDET_early <- sleuth_results$sigDET_Zero_vs_Early
...
for(s1 in names(selectGenes)){
  ...
  hMap1 <- de.heatmap(tpmMat=get(tpmIn), gene_list=selectSigDET, clusterCols = FALSE, 
                      colAnnot=get(paste0("colAnnot_", s1)), 
                      savePath=file.path(experiment_path, paste0(s1, "_heatmp.png")))
