---
title: "Differential Expression Analysis - Juvenile samples"
date: "`r Sys.Date()`"
...
experiment <- "11-16-2023-DE_Analysis_Juvenile" # results with pvalue of 0.01
## experiment <- "06-05-2023-DE_Analysis_Juvenile" # results with pvalue of 0.01
...
metadataPath <- file.path(data_path, "juvenile_samples.csv")
skipLines <- 0
sampleCol <- "Sample"
...
sleuth_results <- sleuth.func(metaData = metadataPath, skipLines=skipLines, sampleCol=sampleCol,
                                     ...
                                     drop_dupGenes = FALSE)
## save(sleuth_results, file.path(,"sleuth_results.RData"))
...
sleuth_results[["Late_vs_Zero_pca"]]
```

#### Volcano Plot

```{r volcano_2-1, echo=FALSE, warning=FALSE, message=FALSE}
sleuth_results[["Late_vs_Zero_vPlot"]]
```
...
sleuth_results[["Early_vs_Zero_pca"]]
```

#### Volcano Plot

```{r volcano_2-3, echo=FALSE, warning=FALSE, message=FALSE}
sleuth_results[["Early_vs_Zero_vPlot"]]
```
...
for(s1 in names(selectGenes)){
  ...
  hMap1 <- de.heatmap(tpmMat=get(tpmIn), gene_list=selectSigDET, clusterCols = FALSE, 
                      colAnnot=colAnnot, savePath=file.path(experiment_path, paste0(s1, "_heatmp.pdf")))
